...
def update_item_from_dict(table, key, dictionary):
    ...
    """
    updates_string = ', '.join([f'#{pattern.sub("", k)} = :{pattern.sub("", k)}' for k in dictionary.keys()])
    update_expression = f'SET {updates_string}'
    attribute_names = {f'#{pattern.sub("", k)}': k for k in dictionary.keys()}
    attribute_values = {f':{pattern.sub("", k)}': v for k, v in dictionary.items()}
